...
# =========================
dialogues = [
    ("Sophia", "Hello, I'm Sophia. What is your name?", "Xin chào, mình là Sophia. Tên bạn là gì?", "female"),
    ("Mike", "Hi, Sophia. My name is Mike.", "Chào Sophia, mình tên là Mike.", "male"),
    ("Sophia", "Nice to meet you, Mike. Is this your first day at this school?", "Rất vui được gặp bạn. Đây là ngày đầu tiên bạn ở trường này à", "female"),
    ("Mike", "Yes, it is. I'm a little nervous.", "Vâng, đúng vậy. Mình có chút hồi hộp.", "male"),
    ("Sophia", "Don't worry. I am new here too.", "Đừng lo lắng. Mình cũng là người mới ở đây.", "female"),
    ("Mike", "Really?", "Thật à?", "male"),
    ("Sophia", "Yes, but I'm excited to start school and make new friends.", "Đúng vậy, nhưng mình rất hào hứng được bắt đầu năm học và kết bạn mới.", "female"),
    ("Mike", "That's great to hear. Where are you from?", "Thật tuyệt khi nghe bạn nói vậy. Bạn đến từ đâu?", "male"),
    ("Sophia", "I'm from New York. How about you?", "Mình đến từ New York. Còn bạn, bạn đến từ đâu?", "female"),
    ("Mike", "I'm from Los Angeles.", "Mình đến từ Los Angeles", "male"),
    ("Sophia", "That's cool! Let's go to class together", "Thật tuyệt! Hãy cùng vào lớp nào.", "female"),
    ("Mike", "Sure. Thanks, Sophia.", "Ừ, Cám ơn, Sophia.", "male"),
    ("Sophia", "You are welcome.", "Có gì đâu.", "female"),
    ("Mike", "Nice to meet you.", "Rất vui được làm quen với bạn.", "male"),
    ("Sophia", "Nice to meet you too.", "Mình cũng rất vui được làm quen với bạn.", "female")
]
KICH_BAN = [
    {
        "nhan_vat": character,
...
INTRO = "Chào tất cả các bạn!\nRất vui được ở đây cùng các bạn.\nChúc các bạn có một ngày thật tốt lành.\nCùng bắt đầu nào!"
hook_vi = "Bạn đã làm quen với ai vào ngày đầu tiên ở ngôi trường mới chưa?"
hook_us = "Did you make any friends on your first day at the new school?"
topic_vi = "Bài 1: Làm quen bạn mới."
topic_us = "Lesson 1: Making new friends."
...
if not (os.path.exists(f"{BASE_DIR}/image/base_scene.png")
    ...
    downloads = ["base_scene.png", "intro.png", "topic.png", "female.png", "male.png", "hook.png"]
    for f in downloads:
        shutil.copy(f"{DOWNLOAD_DIR}/image/{f}", f"{BASE_DIR}/image/{f}")
...
async def audio_topic(topic_vi, topic_us, file_out):
    ...
    silence = AudioSegment.silent(duration=2000)  # 2000 ms = 2 giây
    silence1 = AudioSegment.silent(duration=1000)  # 2000 ms = 2 giây


    # Ghép audio: lần 1 + lần 2
    final_audio = silence1 + clip_vi + silence+ clip_us + silence
    temp_files = [
        file_vi,
        file_us,
    ]
...
async def audio_listen_advice(text_vi, text_us, file_out):
    ...
    silence1 = AudioSegment.silent(duration= 500)  # 2000 ms = 2 giây
...
async def audio_repeat(phan):
    ...
    silence = AudioSegment.silent(duration= 1000)  # 2000 ms = 2 giây
    silence1 = AudioSegment.silent(duration= 1500)  # 2000 ms = 2 giây

    # Ghép audio: lần 1 + lần 2 + silence + lần 3
    final_audio = silence + clip1 +silence + clip2 + silence + clip3 + silence1
    temp_files = [
        # vi_file,
        fast_file,
        slow_file,
...
def draw_text_preview(base_image_path, output_path,text_vi, text_us):
    ...
    # 3. Vẽ tiếng Việt
    draw.multiline_text((vi_x, vi_y), wrapped_vi, font=font_vi,  fill=(0, 70, 220), align="center")
...
async def build_final_video():
    ...
    preview_img = f"{BASE_DIR}/image/base_scene.png"
    ad_img = f"{BASE_DIR}/image/female.png"

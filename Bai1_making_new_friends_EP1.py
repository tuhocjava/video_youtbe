...
def add_speech_bubble(base_image_path, output_path,text, scene):
    ...
    img_cv = cv2.imread("output_5/image/base_scene.png")
    ...
    (a, b, w, h) = faces[0]  # người bên trái nhất

    #======

    # if position == "above_emily_center":
    if scene["sex"] == "female":
        (a, b, w, h) =  faces[1]
        head_x = a + w // 2
        head_y = b + h // 3  # lấy điểm gần trán

        x = head_x - bubble_width  - 70
    ...
    # elif position == "above_john_center":
    elif scene["sex"] == "male":
...
def add_background_music(video, music, output):
    run_ffmpeg([
        ...
        "-filter_complex", "[1:a]volume=0.02[a1];[0:a][a1]amix=inputs=2:duration=first[a]",
        "-map", "0:v", "-map", "[a]",
        "-c:v", "copy", "-c:a", "aac",
        output
    ])

...
# =========================
# dialogues = [
#     ("Emma", "Hi Mike, how are you today?", "Chào Mike, hôm nay bạn khỏe không?", "female"),
#     ("Mike", "I’m good, thanks.", "Mình khỏe, cảm ơn.", "male"),
#     ("Emma", "I’m really happy today!", "Hôm nay mình rất vui!", "female"),
#     ("Mike", "That’s nice. Why are you happy?", "Thật tuyệt. Vì sao bạn vui vậy?", "male"),
#     ("Emma", "Because I listened to my favorite music this morning.", "Vì sáng nay mình đã nghe nhạc yêu thích.", "female"),
#     ("Mike", "Oh, so you like music?", "Ồ, vậy là bạn thích âm nhạc?", "male"),
#     ("Emma", "Yes, I love listening to pop songs.", "Đúng vậy, mình rất thích nghe nhạc pop.", "female"),
#     ("Mike", "Cool! I also like music, but I prefer rock.", "Hay quá! Mình cũng thích nhạc, nhưng mình thích rock hơn.", "male"),
#     ("Emma", "That’s interesting. What else do you like to do?", "Thật thú vị. Bạn còn thích làm gì nữa?", "female"),
#     ("Mike", "I like playing football with my friends.", "Mình thích chơi bóng đá với bạn bè.", "male"),
#     ("Emma", "Sounds fun! Maybe one day I can watch you play.", "Nghe vui đó! Có lẽ một ngày nào đó mình sẽ xem bạn chơi.", "female"),
#     ("Mike", "Sure, and maybe we can listen to music together too.", "Được chứ, và có thể chúng ta cùng nghe nhạc nữa.", "male")
# ]

dialogues = [
    ("Sophia", "Hi Ethan! Do you smell that delicious food from the canteen?",
     "Chào Ethan! Bạn có ngửi thấy mùi đồ ăn thơm ngon từ căng tin không?", "female"),

    ("Ethan", "Hi Sophia! Yes, I do. It smells wonderful!",
     "Chào Sophia! Có chứ. Mùi thật tuyệt vời!", "male"),

    ("Sophia", "The smell is making me hungry.",
     "Mùi hương này làm mình thấy đói rồi.", "female"),

    ("Ethan", "Me too! Let's go to the canteen for lunch.",
     "Mình cũng vậy! Hãy đến căng tin ăn trưa nhé.", "male"),

    ("Sophia", "Great idea! What would you like to eat today?",
     "Ý hay đấy! Hôm nay bạn muốn ăn gì?", "female"),

    ("Ethan", "I think I'll get some sandwiches.",
     "Mình nghĩ mình sẽ ăn vài chiếc bánh sandwich.", "male"),

    ("Sophia", "That sounds tasty. I want a salad and some fruit.",
     "Nghe ngon đấy. Mình muốn ăn salad và một ít trái cây.", "female"),

    ("Ethan", "Look! The lunch line is not too long today.",
     "Nhìn kìa! Hôm nay hàng chờ lấy đồ ăn không quá dài.", "male"),

    ("Sophia", "That's good. We can get our food quickly.",
     "Tốt quá. Chúng ta có thể lấy đồ ăn nhanh hơn.", "female"),

    ("Ethan", "My salad looks fresh, and the fruit looks sweet.",
     "Salad của mình trông rất tươi, và trái cây có vẻ rất ngọt.", "male"),

    ("Sophia", "Wow! These sandwiches taste wonderful!",
     "Wow! Những chiếc bánh sandwich này ngon tuyệt!", "female"),

    ("Ethan", "I agree. The food at the school canteen is really good.",
     "Mình đồng ý. Đồ ăn ở căng tin trường thực sự rất ngon.", "male")
]
...
hook_vi = "Ăn trưa cùng mình ở căng tin nhé"
hook_us = "Join me for lunch at the school canteen"
topic_vi = "Bài 4: Ăn trưa ở căng tin"
topic_us = "Lesson 4: Having lunch at the canteen."
...
# Hàm tạo audio intro.txt-outtro
...
def add_speech_bubble(base_image_path, output_path,text_vi, text_us, scene):
    ...
    if scene["sex"] == "female":
        (a, b, w, h) = faces[0]
        x  = (a + img.width + w - bubble_width)/2 -180
    elif scene["sex"] == "male":
        (a, b, w, h) = faces[1]
...
async def all_preview_audio_imge():
    for i, scene in enumerate(KICH_BAN, start=1):
        ...
        if not os.path.exists(output_image):
            img = draw_text_preview_hook(
                base_image_path=f"{BASE_DIR}/image/base_scene.png" if i > 5 
                else f"{DOWNLOAD_DIR}/image/topic.png",

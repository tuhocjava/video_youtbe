...
hook_vi = "Ăn trưa cùng mình ở căng tin trường nhé"
...
if not (os.path.exists(f"{BASE_DIR}/image/base_scene.png")
        and os.path.exists(f"{BASE_DIR}/image/intro.txt.png")
    ...
    downloads = ["base_scene.png", "intro.txt.png", "topic.png", "female.png", "male.png", "hook.png", "advice.png"]
    for f in downloads:
...
async def all_preview_audio_imge():
    for i, scene in enumerate(KICH_BAN, start=1):
        ...
        if not os.path.exists(output_image):
            img = draw_text_preview(
                base_image_path=f"{BASE_DIR}/image/base_scene.png"  if i > 4
...
async def build_final_video():
    intro_img = f"{BASE_DIR}/image/intro.txt.png"
    ...
    intro_audio = f"{BASE_DIR}/audio/intro.txt.mp3"
    ...
    intro_video = f"{BASE_DIR}/videos/intro.txt.mp4"
